...
Examining sample price paths with the trailing stop and signal superimposed, it is possible to get some intuition about the result. As we increase the strength of long memory in the true range, the price paths get visibly more volatile (See Figure directly above). The stop is the blue line that ratchets behind the price depicted in black. The red line depicts the fast EMA, while the purple depicts the slow EMA. As we reduce the drift (in absolute value terms), the strategy gets stopped out more often. Performance is better on the long side than the short side because price is unbounded on the positive side, but bounded by zero on the negative side.
